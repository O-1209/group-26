...
marvel <- read.csv('/Users/zhaozeyu/Documents/R programm code/data analysis skill/marvel.csv')
...
+ Variables explanation  
  ...
  Gender(binary variable): female, male and unknown(as some of the heroes are not humanity) *-explanatory variable*  
...
#Star plot
marvel.Star <- read.csv('/Users/zhaozeyu/Documents/R programm code/data analysis skill/marvel.csv')
...
$Y_{Combat} = \alpha + \beta_{1} * X_{Gender} + \beta_{2} * X_{Height} + \beta_{3} * X_{Weight} + \beta_{3} * X_{Intelligence} + \beta_{4} * X_{Strength} + \beta_{5} * X_{Speed} + \beta_{6} * X_{Durability} + \beta_{7} * X_{Power} + \epsilon$
...
From the regression table, we noticed that half of the parameters in the full model are not significant due to the multicollinearity, thus the stepwise regression is used to address this problem. 
...
+ Checking the influencial point
...
## Model checking using other method of stepwise selection 
  ...
  In the above disscussion, we use the backward stepwise regression to select the virables. Now we use another method of stepwise regresstion, which is combining forward and backward selection, to check whether the model from backward is the same as the model from combining forward and backward selection.
...
# Summary {#sec:Sum}
  ...
  The final model is$Y_{Combat}=3.396 + 0.040* X_{Height}+0.696* X_{Intelligence}+0.130* X_{Durability} + \epsilon$. That means there are three statistical significant charactisitcs of hero (height, intelligence and durability), and these variables are a positive impact on the combat power. To be specific, when increasing one unit of Height value will increase the combat power of the hero by 0.04. When increasing one unit of Intelligence score will lead to increase the combat power of the hero by 0.696 and one unit of Durability score will cause increase by 0.13 of combat score. In Addition, The $R^2$ is 63.8%, it is already quite plausible to build a simple linear model here, 



because the realistic data always have complex relationships between the explanatory variables. But if considering the multicollinearity, the model might be improved a lot, so maybe a general linear model is more suitable for the realistic data research.
