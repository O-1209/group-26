...
cor <- cor(marvel[,-c(1,2)])
corrplot(corr = cor , method ='circle' ,type='upper',order='hclust') #cor matrix, this plot is same as the 
```
...
$Y_{Combat} = \alpha + \beta_{1} * X_{Gender} + \beta_{2} * X_{Height} + \beta_{3} * X_{Weight} + \beta_{3} * X_{Intelligence} + \beta_{4} * X_{Strength} + \beta_{5} * X_{Speed} + \beta_{6} * X_{Durability} + \beta_{7} * X_{Power}$
...
The final model is$Y_{Combat}=\alpha+ \beta_1* X_{Height}+\beta_2* X_{Intelligence}+\beta_3* X_{Durability}$.The $R^2$ is 63.8%, it is already quite good for a simple linear model here, because the realistic data always have complex relationships between the explanatory variables. But if considering the multicollinearity, the model might be improved a lot, so maybe a general linear model is more suitable for the realistic data research.
